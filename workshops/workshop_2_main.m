...
%     %I would say use the rates csv to then check for the velocity term
...
% 
% end
%% Assuming omega_ie and c are defined earlier in the code or in an included constants file

% Hardcoded satellite numbers for example
sat_nos = [4, 5, 9, 14, 15, 19, 20, 24, 29, 30];
num_satellites = length(sat_nos);
...
for n = 1:num_satellites
    ...
    % Omega_ie = [0 -omega_ie 0; omega_ie 0 0; 0 0 0]; % Skew-symmetric Earth rotation rate matrix
...
% 
% %R measurement noise covariance matrix is different - doesnt match workshop
...
mat_sigp = eye(10) * 10^2;
mat_sigr = eye(10) * 0.05^2;

R = [mat_sigp zeros(10,10);
        zeros(10,10) mat_sigr];
...
P_est1 = (eye(8) - K*H)* P_est;  %% This is perfectly correct
