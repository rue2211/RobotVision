function [state_est, P_est, latitude, longitude, height, vel] = GNSSImp(prev_state, prev_P_est, r)
    %     %%
...
Omega_ie = Skew_symmetric([0,0,omega_ie]);
    ...
    mat_sigp = eye(8) * 10^2;
    mat_sigr = eye(8) * 0.05^2;
    
    R = [mat_sigp zeros(8,8);
    zeros(8,8) mat_sigr];
    ...
    delta_z_minus(n) = measured_pseudo_range_row(n) - predicted_ranges(n) - state_est(7);
    end

    % Storing delta_zs for pseudo range rates (clock drift)
    for n = num_satellites+1:num_satellites*2
        i = n - num_satellites;
        delta_z_minus(n) = measured_pseudo_range_rate_row(i) - predicted_range_rates(i) - state_est(8);
    end
